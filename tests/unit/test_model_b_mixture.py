...
from scripts.build_model_b_mixture import sample_epoch_ids
...
def test_rotation_is_deterministic_balanced_and_complete() -> None:
    ...
    first, replaced = sample_epoch_ids(gold, pseudo, seed=2026, num_epochs=3, pseudo_per_epoch=8372)
    second, _ = sample_epoch_ids(gold, pseudo, seed=2026, num_epochs=3, pseudo_per_epoch=8372)
...
def test_small_pool_records_replacement() -> None:
    epochs, replaced = sample_epoch_ids(["g1"], ["p1", "p2"], seed=2026, num_epochs=1, pseudo_per_epoch=3)

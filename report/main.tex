...
\subsection{Controlled pool-by-duration follow-up}

To separate weak-label pool availability from training duration, a later Dev-only factorial follow-up crossed the initial 18,970-label pool and the expanded 64,813-label pool with training horizons of three and eight epochs. The four cells are denoted S3, S8, L3, and L8, where S and L indicate the small and expanded pools. Three paired seeds were used. Each epoch retained 8,372 gold and 8,372 pseudo examples, the gold-to-pseudo ratio, Model~A initialization, tokenizer, sequence limits, batch configuration, learning rate, warmup ratio, and generation settings. Within each horizon, the checkpoint with the lowest Dev loss was selected; ERR was the primary comparison metric. Terminal checkpoints were retained only for sensitivity analysis. ViLexNorm Test was not loaded for this follow-up.

\begin{table}[t]
\centering
\caption{Controlled factorial design.}
\label{tab:factorial-design}
\begin{tabular}{lrr}
\toprule
Weak-label pool & 3-epoch horizon & 8-epoch horizon \\
\midrule
Initial: 18,970 & S3 & S8 \\
Expanded: 64,813 & L3 & L8 \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{Controlled factorial follow-up}

Table~\ref{tab:factorial-results} reports means over the three paired seeds using the frozen lowest-Dev-loss checkpoint rule. L8 is the strongest cell under this protocol. In contrast, the expanded pool does not improve the selected checkpoint at the three-epoch horizon. Its duration gain is larger than the small pool's, yielding a positive pool-by-duration interaction.

\begin{table*}[t]
\centering
\caption{Dev-only controlled factorial results, averaged over three seeds. Checkpoints are selected by lowest Dev loss within each frozen horizon.}
...
L8 & \textbf{0.670380} & \textbf{0.761249} & \textbf{0.563175} \\
\bottomrule
\end{tabular}
\qquad
\begin{tabular}{lrrr}
\toprule
Effect & ERR & F1 & Exact match \\
\midrule
Pool at 3 epochs & -0.003713 & -0.002936 & -0.005714 \\
Pool at 8 epochs & +0.007701 & +0.007067 & +0.009524 \\
Duration, initial pool & +0.009901 & +0.006887 & +0.004762 \\
Duration, expanded pool & +0.021315 & +0.016889 & +0.020000 \\
Interaction & +0.011414 & +0.010002 & +0.015238 \\
\bottomrule
\end{tabular}
\end{table*}

Terminal-checkpoint sensitivity attenuates the L8--S8 difference to near zero. Thus, L8 is strongest under the frozen Dev-loss checkpoint-selection protocol, rather than evidence of a large unconditional terminal-epoch pool effect. The factorial conclusion is recorded as artifact FCT-CONCLUSION.

\subsection{Exploratory maximum-20-epoch optimization}

After identifying Model~C/L8, trained with the expanded pool for eight epochs, as the strongest configuration under the frozen checkpoint-selection protocol, the group conducted an additional exploratory run to examine the performance attainable under a larger training budget. This run used the expanded pool with seed 2026, a maximum of 20 epochs, a minimum of 8 epochs, Dev-loss patience 4, and minimum improvement 0.0001. It stopped at epoch 13 and selected epoch 9 by Dev loss. The selected checkpoint did not improve the L8 selected-checkpoint mean. The terminal checkpoint has higher F1 and exact match, but it is not selected by the frozen Dev-loss rule and is sensitivity evidence only. This run is artifact OPT-CMAX20 and is not promotion eligible.

\begin{table}[t]
\centering
\caption{Exploratory C-max20-ES Dev-only result for seed 2026.}
\label{tab:cmax20-results}
\begin{tabular}{lrrrr}
\toprule
Checkpoint & Epoch & ERR & F1 & Exact match \\
\midrule
Selected by Dev loss & 9 & 0.668729 & 0.759552 & 0.560952 \\
Terminal at stop & 13 & 0.669554 & 0.765908 & 0.567619 \\
\bottomrule
\end{tabular}
\end{table}
...
Relative to Model~A, Model~B increased F1 by 0.024030. Relative to Model~B, Model~C increased F1 by 0.022778 and increased ERR by 0.031614. For the Model~C--Model~B comparison, the bootstrap 95\% interval for the F1 difference is $[0.012063, 0.033228]$, while the corresponding ERR interval is $[0.015893, 0.046803]$. This is a descriptive benchmark because the Test split had already been observed before the later factorial and C-max20 follow-ups.

The common Test results descriptively rank Model~C highest among the historical A/B/C configurations. They were not used for the later factorial conclusion, C-max20 stopping rule, checkpoint selection, or application-promotion decision. The application continues to use the previously Dev-selected Model~C checkpoint with Model~B as fallback.
...
The historical B/C comparison changes pool size and training duration simultaneously. The later factorial follow-up reduces this particular confounding, but does not make pool size the only difference: the expanded pool also changes source, domain, and reviewed-label composition. Moreover, Dev is used both for checkpoint selection and reporting, selected and terminal checkpoints can differ, and three seeds provide limited precision for seed-level variation.

The C-max20 run is exploratory and uses one seed with a different maximum-horizon learning-rate schedule. Its selected and terminal metrics therefore do not justify an application promotion or a revision of the frozen factorial conclusion. Finally, the previously observed Test split is not an independent confirmatory holdout; a newly frozen holdout is required for a stronger final scientific claim.
...
This study compared BARTpho-syllable configurations for Vietnamese social media lexical normalization. Historical A/B/C benchmarking ranks Model~C highest descriptively on the previously observed Test split. The later three-seed factorial follow-up identifies L8 as the strongest configuration under the frozen Dev-loss checkpoint-selection protocol and indicates that the expanded pool benefits more from a longer training horizon than the initial pool.

Model~C remains the default local application checkpoint, while Model~B remains the verified fallback. The exploratory C-max20 run did not justify a promotion. Future work should expand human auditing and evaluate the frozen application checkpoint on a newly frozen independent holdout.

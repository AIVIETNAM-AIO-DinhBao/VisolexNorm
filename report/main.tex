...
\title{Vietnamese Social Media Lexical Normalization with BARTpho:\\
A Comparative Study of Gold and Reviewed Weak-Label Training}
\author{}
\date{}
...
\begin{abstract}
Vietnamese social media text frequently contains abbreviations, informal spellings, omitted diacritics, and other non-standard lexical forms. This study formulates their normalization as a sequence-to-sequence task and compares three BARTpho-syllable configurations. Model~A was fine-tuned on 8,372 human-annotated ViLexNorm training pairs. Model~B was initialized from Model~A and trained with a balanced mixture of gold data and an initial pool of 18,970 reviewed weak labels. Model~C was also initialized from Model~A but used an expanded pool of 64,813 reviewed weak labels. The weak labels were constructed by generating candidate normalizations for ViSoLex sentences, reviewing candidates under a \textsc{Keep}, \textsc{Edit}, or \textsc{Reject} protocol, and applying deterministic validation rules. Under a common 1,045-sentence ViLexNorm Test evaluation, Models~A, B, and C obtained F1 scores of 0.7182, 0.7422, and 0.7650, respectively. Model~C achieved the highest result and was selected for a local Gradio application, while Model~B was retained as an available fallback checkpoint. The application performs BARTpho inference locally and does not use the review model at runtime.
\end{abstract}
...
The available supervised resource, ViLexNorm, provides human-annotated pairs of noisy and normalized Vietnamese social media sentences \citep{nguyen2024vilexnorm}. Its 8,372-example training split provides a direct basis for supervised fine-tuning, but social media language is diverse and its lexical variation is not fully represented by a moderate-sized gold corpus. In contrast, the processed ViSoLex corpus used in this project contains 68,411 unlabeled social media sentences. The central question of this study is therefore whether reviewed labels constructed from this unlabeled corpus can provide additional training signal for a BARTpho model.

The study compares three configurations of the same BARTpho-syllable architecture. Model~A uses only ViLexNorm gold training data. Model~B continues from Model~A with an initial reviewed weak-label pool of 18,970 examples. Model~C also continues from Model~A but uses an expanded reviewed weak-label pool of 64,813 examples. The comparison is designed to examine how the composition and coverage of reviewed weak-label data are associated with lexical-normalization performance. The project further implements a local Gradio application using the selected checkpoint, so that inference does not require the training corpus or an external review API.
...
The additional unlabeled corpus was built from ViSoLex sources. After whitespace normalization, removal of empty records, exact deduplication, and leakage filtering, the canonical corpus contains 68,411 sentences. Provenance is retained through an \texttt{original\_source} field. ViHOS has zero retained examples because its records were exact duplicates of ViHSD records loaded earlier in the pipeline; this value does not mean that the original ViHOS source was empty.
...
Each candidate was reviewed under a frozen instruction and a fixed structured response schema. A \textsc{Keep} decision accepts the Model~A candidate as the target. An \textsc{Edit} decision replaces it with a minimally corrected target supplied by the reviewer. A \textsc{Reject} decision excludes the record because a reliable lexical target cannot be determined or because the transformation lies outside lexical normalization. The reviewer is used only in offline data preparation. It is not loaded or called by the deployed application.
...
The initial accepted pool defines the additional data available to Model~B. The union of the initial and expanded accepted pools defines the additional data available to Model~C. Model~A uses neither pool during its supervised fine-tuning. The three data conditions thus differ in reviewed weak-label coverage while preserving a common human-labeled ViLexNorm training source.
...
The project also reports ERR using the Leave-As-Is interpretation,
\begin{equation}
ERR=\frac{C}{G}.
\end{equation}
Under this token-edit implementation, Leave-As-Is resolves no gold edits, so ERR is numerically equal to edit recall. This equality is a property of the current implementation and should not be generalized to every ERR definition. Exact sentence match is additionally reported as the proportion of sentences whose generated output equals the reference target exactly.

All three configurations are evaluated on the same ViLexNorm Test split containing 1,045 sentences. The tokenizer contract, generation settings, prediction schema, and edit-based scoring procedure are held constant across Models~A, B, and C. This common protocol compares the three training configurations under consistent inference and evaluation conditions. Bootstrap intervals for pairwise metric differences use 2,000 resamples and seed 2026.
...
\midrule
Model A & 1,778 & 1,703 & 1,250 & 0.703037 & 0.733999 & 0.718184 & 0.503349 \\
Model B & 1,778 & 1,690 & 1,287 & 0.723847 & 0.761538 & 0.742215 & 0.529187 \\
Model C & 1,778 & 1,707 & 1,333 & 0.749719 & 0.780902 & 0.764993 & 0.566507 \\
\bottomrule
\end{tabular}}
\end{table*}

Relative to Model~A, Model~B increased F1 by 0.024030. Relative to Model~B, Model~C increased F1 by 0.022778 and increased ERR by 0.025872. For the Model~C--Model~B comparison, the bootstrap 95\% interval for the F1 difference is $[0.012063, 0.033228]$, while the corresponding ERR interval is $[0.013296, 0.038206]$. These intervals are positive under the stated resampling procedure and support the higher performance of Model~C in the common evaluation.
...
\subsection{Error-oriented interpretation}

The edit formulation makes it possible to distinguish several failure patterns. A missed normalization occurs when the prediction remains identical to the noisy input although the reference contains a lexical change. Over-normalization occurs when the input already equals the reference but the model changes it anyway. Other cases involve one-to-many and many-to-one transformations, which are particularly relevant to abbreviations and compressed social-media forms. The project error-analysis code also retains a suspected weak-label-noise category, but it assigns that category only when explicit supporting evidence is available; uncertain examples are otherwise counted as wrong.

The aggregate results suggest that the progression from Model~A to Model~C is not merely a consequence of producing more edits. Model~B predicts slightly fewer edits than Model~A while obtaining more correct edits, which is reflected in higher precision. Model~C predicts slightly more edits than Model~B but also increases the number of correct edits by 46, raising both precision and recall. A qualitative appendix can provide aligned input, reference, and three-model outputs for individual examples, but such examples should be interpreted as illustrations rather than as substitutes for aggregate evaluation.

\section{Local Web Application}

The application is implemented with Gradio and runs on the local loopback address \texttt{127.0.0.1}. The deployed default checkpoint is Model~C because it achieved the highest values in the common Test comparison. Model~B is retained as an available fallback checkpoint.
...
The reviewed weak labels are not equivalent to human annotations. The review protocol, fixed prompt, schema validation, and deterministic filters reduce identifiable problems, but they do not guarantee that every accepted target is semantically optimal. Some lexical variants are context-dependent, and an automated reviewer may select a plausible target that differs from the preferred annotation in ViLexNorm-style evaluation.
...
This study compared three BARTpho-syllable configurations for Vietnamese social media lexical normalization. Model~A used only human-annotated ViLexNorm data, Model~B added an initial pool of 18,970 reviewed weak labels, and Model~C used an expanded pool of 64,813 reviewed weak labels. Development loss decreased from Model~A to Model~C, and the common benchmark produced F1 values of 0.718184, 0.742215, and 0.764993 for Models~A, B, and C, respectively. The results are consistent with reviewed social-media examples providing useful additional training variation.

Model~C achieved the highest values in the common Test evaluation and is used as the default local application checkpoint, while Model~B remains available as a fallback. Future work should expand human auditing of accepted weak labels and separate the effects of training duration, initialization, and weak-label coverage more directly.
